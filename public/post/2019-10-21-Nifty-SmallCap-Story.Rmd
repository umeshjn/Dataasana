...
smallcap <- read_excel("Nifty Smallcap 250 Components.xlsx") 
smallcap <- smallcap %>% 
            clean_names()
...
hchart(smallcap1Year$Bucket, color = "#590307") %>%
  ...
  hc_subtitle(text = "93 Stocks have given positive returns out of which 8 stocks have given more than 75% returns. \n 156 stocks have given negative returns out of which 5 stocks have eroded more than 90% of their stock value")

```

<br>

There have been lot of companies which have got into trouble due to mismanagement and unethical business practices specially in last 18-20 months. For every bad news these stocks are getting punished severely and lot of companies have lot huge market caps. Indian stock market is filled with lot of such companies, as lot of experienced investors say if retail investors get into such companies we are bound to lose the capital. Whether it is today or tomorrow that market will decide but for sure we will be punished badly beyond recovery.
...
hchart(bottom5, "column", hcaes(name, x1_year), color = "#590307") %>%
  ...
  hc_subtitle(text = "No Surprise with the names. More than the kind of business it was the mismanagement of these firms which led to this erosion of wealth")
...
smallcap1Year$PL <- cut(
  smallcap1Year$x1_year,
  breaks = c(-99, 0, 160),
  labels = c("Total_Loss%", "Total_Gains%"),
  right  = FALSE
)

worstisnotover <- smallcap1Year %>%
  group_by(PL) %>%
  summarise(Total = sum(x1_year))

hchart(worstisnotover, "column", hcaes(PL, Total), color = "#590307") 

```

</div>

Key insights which gives us the extent of mayhem on small caps in the Indian Market.

* 3000% gains in the stocks which have given positive returns. 

* 4600% loss in the stocks which have given negative returns.

This shows there is a huge damage to the wealth of small cap investors. Investing in small caps is always high risk and people who invest should be fully aware of it. But vast majority of the investors who get into these stocks buy them based on the news and suggestions by analysts or by looking at the daily chart movements. Without understanding the fundamentals of the company and its business if someone is betting then they are bound to lose their money. This has happened 20-30 years back, its happening today and will keep happening in years to come as well.
